...
type VideoTableProps = {
  ...
  setChannelVideosState: React.Dispatch<
    React.SetStateAction<ChannelVideosResponse>
  >;
...
const VideoTable: React.FC<VideoTableProps> = ({
  ...
  const toggleIsPublic = async (videoId: string): Promise<void> => {
    try {
      ...
      if (data.success) {
        for (let i = 0; i < channelVideosState.docs.length; i += 1) {
          if (channelVideosState.docs[i]._id === videoId) {
            channelVideosState.docs[i].isPublic =
              !channelVideosState.docs[i].isPublic;
